...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- variables -->
    <xsl:variable name="collection" select="'temp_alias_001'"/>
    <!-- [!] UPDATE CDM ALIAS / URL WILL CHANGE FOLLOWING ASSIGNMENT OF CDM ALIAS -->
    <!-- [!] UPDATE CDM ALIAS / schemasProject > properties-files [!] -->
    <xsl:variable name="tjls-map"
    ...
    <xsl:variable name="default-co-prop"
        select="'^p88$|^p45$|^p47$|^p23$|^p21$|^p26$|^p27$|^p85$|^p128$|^p136$|^p137$|^p147$|^p53$|^p50$|^p28$|^p30$|^p59$|^p77$|^p75$|^p71$|^p44$|^p90$|^p57$|^p11$'"/>
    <xsl:variable name="default-coitem-prop"
        select="'^p88$|^p28$|^p30$|^p71$|^p90$|^p37$|^p29$|^p62$|^p31$|^p134$'"/>
            ...
            <html>
                ...
                <body>
                    ...
                    <xsl:call-template name="all-list-start">
                        <xsl:with-param name="tjls-map"
                            select="$tjls-map/mig:migDataDictionary/mig:properties"/>
                    </xsl:call-template>
                    ...
                    <xsl:call-template name="CC0-footer">
                        <xsl:with-param name="resource_title">
                            ...
                            <xsl:value-of select="$tjls-map/mig:migDataDictionary/mig:ddName"/>
                            <xsl:text> - </xsl:text>
        ...
        <div class="large_one">
            <ul>
                <li>
                    ...
                    <xsl:value-of
                        select="$tjls-map/mig:migDataDictionary/mig:originalCreationDate"/>
                </li>
                <li>
                ...
                <li>
                    ...
                    <ul>
                        ...
                        <xsl:text>Erika Bailey - &lt;a href="mailto:erikaab@uw.edu"&gt;erikaab@uw.edu&lt;/a&gt;</xsl:text>
                    </ul>
                </li>
                <li>
                    ...
                    <ul>
                        <xsl:for-each
                            select="$tjls-map/mig:migDataDictionary/mig:metadataLiaisons/mig:metadataLiaison">
                            <li>
                                <xsl:value-of select="."/>
                            </li>
                        </xsl:for-each>
    ...
    <xsl:template name="all-list-start">
        ...
        <ol>
            <xsl:for-each select="$tjls-map/mig2:property">
    ...
    <xsl:template name="prop-guidance-section-start">
                ...
                <tr valign="top">
                    <td width="50%">
                        <ul class="no_bullets large_one">
                            <xsl:for-each
                                select="$tjls-map/mig2:property[matches(mig2:uid, $default-co-prop)]">
                                <li>
                    ...
                    <td width="50%">
                        <ul class="no_bullets large_one">
                            <xsl:for-each
                                select="$tjls-map/mig2:property[matches(mig2:uid, $default-coitem-prop)]">
                                <li>
        ...
        <xsl:call-template name="prop-guidance-section-tables">
            <xsl:with-param name="tjls-map" select="$tjls-map"/>
    ...
    <xsl:template name="prop-guidance-section-tables">
        ...
        <xsl:for-each
            select="$tjls-map/mig2:property[matches(mig2:uid, $list-per-resource-and-object-type)]">
            <br/>
