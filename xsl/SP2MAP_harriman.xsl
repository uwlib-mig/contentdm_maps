...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- variables -->
    <xsl:variable name="collection" select="'harriman'"/>
    <xsl:variable name="harriman-map"
        select="document('../../schemasProject/dataDictionaries/xml/harriman-map.xml')"/>
        ...
        <xsl:result-document href="{concat('../html/', $collection, '.html')}">
            <html>
                <head>
                    ...
                    <title>{$harriman-map/migdd:migDataDictionary/migdd:ddName, ': metadata application profile'}</title>
                ...
                <body>
                    ...
                    <xsl:call-template name="all-list-start">
                        <xsl:with-param name="harriman-map"
                            select="$harriman-map/migdd:migDataDictionary/migdd:properties"/>
                    </xsl:call-template>
                    ...
                    <xsl:call-template name="CC0-footer">
                        <xsl:with-param name="resource_title">
                            ...
                            <xsl:value-of
                                select="$harriman-map/migdd:migDataDictionary/migdd:ddName"/>
                            <xsl:text> - </xsl:text>
    ...
    <xsl:template name="top">
        <h1 class="title_color" id="top">
            <xsl:text>UWL MIG CONTENTdm Metadata Application Profile:</xsl:text>
            <br/>
            <xsl:value-of select="$harriman-map/migdd:migDataDictionary/migdd:ddName"/>
            <br/>
        ...
        <div class="large_one">
            <ul>
                <li>
                    ...
                    <xsl:value-of
                        select="$harriman-map/migdd:migDataDictionary/migdd:originalCreationDate"/>
                </li>
                <li>
                ...
                <li>
                    ...
                    <ul>
                        <xsl:for-each
                            select="$harriman-map/migdd:migDataDictionary/migdd:metadataLiaisons/migdd:metadataLiaison">
                            <li>
                                <xsl:value-of select="."/>
                            </li>
                        </xsl:for-each>
    ...
    <xsl:template name="all-list-start">
        ...
        <ol>
            <xsl:for-each select="$harriman-map/migprop:property">
                <li>
    ...
    <xsl:template name="all-list-details">
        ...
        <xsl:text>[ </xsl:text>
        <!-- TO DO - IDS GENERATED HERE - MUST MATCH LATER -->
    ...
    <xsl:template name="prop-guidance-section-start">
        ...
        <div class="{concat($resource-type, '_color large_two')}">
            ...
            <h2 id="{concat($resource-type, '-', $object-type, '-guidance')}">
                <xsl:text>CREATING METADATA FOR STANDALONE OBJECTS</xsl:text>
            </h2>
            <br/>
        </div>
        <xsl:call-template name="prop-guidance-section-tables">
            <xsl:with-param name="harriman-map" select="$harriman-map"/>
    ...
    <xsl:template name="prop-guidance-section-tables">
        ...
        <xsl:for-each select="$harriman-map/migprop:property">
            ...
            <table class="prop_table">
                <thead>
                    <tr>
                        <th colspan="2" id="{concat(migprop:uid, '-guidance')}"
